...
| APN Management, WAN profiles, `cellular/apn.sh`, `apn_mgr.sh` (shared APN lib: v2 config I/O + COPS apply + `reapply_active_apn_slot` + `reconcile_active_apn_slot_at_boot`), `apn_profiles.json`, 5-slot radio model, save/activate/deactivate/clear actions, `active=0` carrier-default state, honest "Not live" badge, COPS detach/attach cycle, IMS/SOS context tagging (not exclusion), CID picker AlertDialog confirm, boot reconcile (poller-hosted, idempotent, `active=0` no-op) | [`docs/features/apn-management.md`](docs/features/apn-management.md) |
| Band Locking, `bands/current.sh` (per-category register query: `lte_band`/`nsa_nr5g_band`/`nr5g_band`/`nrdc_nr5g_band`), `bands/lock.sh`, `qmanager_band_failover`, SA `nr5g_band` vs NR-DC `nrdc_nr5g_band` AT param mapping, SA grep-hazard guard, `qcmd` echo-line grep-anchor invariant, SA⇄NR-DC swap UX, `policy_band` coloring-only (not locked source), `supported_bands.env` 4-field contract, "no zero-band lock" invariant, `supported_bands_hw.env` static band universe (force-copied on upgrade), `hw_lte_bands` / `hw_nsa_nr5g_bands` / `hw_sa_nr5g_bands` poller fields, two-tone primary/warning checkbox split, `policyBands` coloring-only / `supportedBands` universe for count + reset + all-unlocked, NR-DC borrows SA HW list, `refresh_policy_band()` SIM-swap re-read, `/tmp/qmanager_refresh_policy_band` flag | [`docs/features/band-locking.md`](docs/features/band-locking.md) |
