...
- **Brand colors**: Signal Indigo (`--primary`) is the only *action* accent. **Spectrum Teal** (`--brand-teal`, `oklch(0.74 0.13 195)`) is the *identity* color — login hero, wordmark, empty-state art, the indigo→teal gradient — and never a status, button, selected state, or chart color (governing rule: *Indigo acts · Teal identifies · Mono speaks for the machine*). Teal text/icons use `--brand-teal-deep` for AA.
